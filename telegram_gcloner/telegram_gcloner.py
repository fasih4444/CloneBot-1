...
def main():
    ...
    updater.bot.send_message(
        chat_id=config.USER_IDS[0], text='Welcome to ⚡️ MsgSuite clbt ⚡️.\n Let\'s clone some data to your Team Drives !')
    updater.idle()
...
def init_logger():
    ...
    this_file_name = os.path.basename(
        os.path.splitext(os.path.basename(__file__))[0])

    Path('./logs/').mkdir(parents=True, exist_ok=True)
    logfile = './logs/' + this_file_name

    file_logger = handlers.TimedRotatingFileHandler(
        logfile, encoding='utf-8', when='midnight')
    ...
    logging.getLogger(
        'telegram.vendor.ptb_urllib3.urllib3.connectionpool').setLevel(logging.INFO)
